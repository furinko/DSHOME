...
//
// ── 任务可自选「工作区」（2026-09-24 加）───────────────────────────────────
//   任务字段 `workspace`（`cron.cjs resolveRunTarget`）：绝对路径 ⇒ 该自治会话**在它里面跑并归到它**
//   （上游 `attachSession` 硬校验会话 cwd == 工作区路径 ⇒「归属」与「运行目录」必然同一路径）；
//   `'@none'` ⇒ 明确不登记（落「未分组」）；缺省 ⇒ 旧行为（`cwd` + 按 cwd 反查）。
//   处方**不写死**该字段（seed 出来的实例沿用 `cwd` 的旧行为，避免"本机没注册这个工作区就失败"）；
//   使用者在面板「🗂 工作区」里按需选。
'use strict';
