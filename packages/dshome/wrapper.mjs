...
//
// ⚠️ 历史调试探针（E:\DSH 时代环境）：下方 import 硬编码旧环境路径
// `file:///E:/DSH/app.src/lib/desktop-cli.js`（新环境 E:\DSHOME 下不存在），
// 本文件已废弃，仅作参考；生产启动路径见 开发启动.cmd / shell-app。
...
process.env.DSH_HOME = process.env.DSH_HOME ?? join(process.env.USERPROFILE ?? process.env.HOME ?? '.', '.dsh');
