...
>
> **⚠️ 归档说明（2026-08-31）**：本自研方案已被市场插件 **dsh-evolve v0.4.2** 替代——存储/检索/反射/技能固化设计已由 dsh-evolve 实现并实测通过（冒烟全绿，见 `docs/integrations/EVOLVE-SMOKE.md`），本方案相应部分**作废**。
> 仅以下内容可复用：§4.1 AGENTS.md 模板（灵魂纪律层）、§4.5 Learn.md 行为学习模板（**鱼鱼**形象，非猫猫）、§2.5 与 compaction/goal 的分工边界。行为层落地见 `docs/soul/SOUL-BEHAVIOR.md`。
>
> **v1.1 变更记录（评审合入）**：
