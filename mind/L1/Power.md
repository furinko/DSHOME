...
> 版本：1.13 | 2026-09-11 | §二 补「Skill 触发 = 精确子串匹配」实测边界（口语变体须显式列；精确是为防自触发，勿改通配） | 1.12 | 2026-09-11 | 机制名表述统一 | 1.11 | 2026-09-09 | 记忆层重构：沉淀落点路径改 `L3\common\<主题>`（通用教训）/ `L3\projects\<项目>\project.md`+`知识\`（项目）；L3/index 引用清理 | 1.10 | 2026-09-08 | §二 触发机制补全：加载器扫**四区**（Skill 双区 + Exp 双区），Exp 条目自动触发；余同 1.9
...
_版本：1.13 | 2026-09-11 | §二 补「Skill 触发 = 精确子串匹配」实测边界（口语变体须显式列；精确是防自触发的有意设计，勿改通配） | 1.12 | 2026-09-11 | 机制名表述统一 | 1.11 | 2026-09-09 | 记忆层重构：沉淀落点路径改 L3\common / L3\projects\知识；L3/index 引用清理 | 1.10 | 2026-09-08 | §二 触发机制补全：加载器扫四区（Skill 双区 + Exp 双区），Exp 条目按 contract.triggers 自动触发；§二 实现状态三条文案对齐（扫四区 / 能力积木双区 / Exp 落点与触发）| 1.9 | 2026-09-08 | §三 增「自主收录授权（L2 生长区免放行）」：触发时机/落点表/实体与收录分离/刹车/不照抄记忆契约/跨设备对齐 V4 + 补「新增 Exp」流程 + 新增 Skill step2 改「自主收录判定」；§二 补 Exp 落点 | 1.8 | 2026-09-05 | 通用筛补专项/通用判据 + DSHOME 自身技能直接出厂例外；余同 1.7（四筛法）_
