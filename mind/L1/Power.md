...
> 版本：1.6 | 2026-09-05 | 补私有 vs 出厂判定三筛法（隐私/通用/成熟）；余同 1.5（能力手册+机器加载+双区）
...
> **私有 vs 出厂判定（三筛法，2026-09-05）**——蒸馏出新方法论时按序问：
> 1. **隐私筛**（任一"是"→ **必私有**，不讨论）：推上公开 GitHub 会暴露——本机私人路径/用户名/邮箱（`C:\Users\<名>\`、私人系统目录如 Hermes）？公司/个人项目内幕？凭据/密钥/令牌（含示例）？※ `E:\DSHOME\` 是公开项目路径（仓库本体），不算隐私。
> 2. **通用筛**（"否"→ 私有）：换台干净设备/给别人，这份方法论还能照做吗？依赖本机独有环境（特定目录/软件/账号）或个人工作习惯 → 私有；跨项目可复用的"怎么做某类事"逻辑 → 出厂候选。
> 3. **成熟筛**（未验证 → 先私有暂存）：跨 2+ 项目验证过、逻辑闭环稳定 → 出厂；刚蒸馏/可能大改/只 1 项目试过 → 先私有，验证后提升出厂。
> **关键认知**：私有区是**过渡暂存区**，不是藏私区——不成熟/含隐私的 skill 暂存私有，一旦去敏+验证充分就该提升出厂（否则私有区变永久杂物间）。"含路径"≠"含隐私"（项目公开路径可出厂，个人私人路径必私有）。
...
_版本：1.6 | 2026-09-05 | v1.5（机器加载+双区）→ 补私有/出厂判定三筛法（隐私筛必私有·通用筛·成熟筛）_
