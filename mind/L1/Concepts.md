...
> **新设备首启自举（B3 闭环 2026-09-05）**：`mind-prime` 上工召回时若无 `Project\DSHOME\project.md` → **当场按 `mind\L1\Memory.md` §四 模板初始化一份**（含「进度状态」表 + 「下一步」`- [ ]` 列表，标题不带序号前缀），再装配——不等收工（收工 step1 假定档已存在，鸡生蛋由此解开）；mind-prime 无档时目前容错返回空，初始化动作由鱼鱼在首会话开场补做。
