...
|---|---|---|---|---|---|
| `verify-integrity` | 验证可信度四则：真加载（语言层查不出"东西不存在"）/ 反例证伪（写不出反例=没验过）/ 不污染被测对象 / 输入缺失即响亮失败 | 验证 / 自测 / 怎么确认生效 / 测试全绿 / 门禁可信 / 裁判本身 / 反例 / 评分 / 打分 / 对比评分 / 判据过时 / 凭据泄漏 | 待验证机制+已有手段 → 四则判据 + 反例设计 + 前置检查 + 响亮失败改造（+5 配方：判别"线上跑的是不是新代码"、门禁探针零风险设计、调参改动先离线扫参数矩阵、评分/自评三件套、判据也是消费者＝同批改判据 + 复刻用户路径 + 诊断打码） | landing-audit · verify-host-plugins · verify-guard-decisions | 1.5.0 |
...
| `concurrent-writers` | 多写者共享工作区的纪律：先盘点写者（"干净"≠"没别人"）/ 共享配置单写者、并行走独立环境 / 台账 append-only / 拿不到闸就响亮失败 / 提交前复核 staged / 善后先判谁覆盖了谁 | 并发 / 多写者 / 同时改 / 抢写 / 写冲突 / 共享工作区 / 单写者 / 互斥 / 串行 / 排队 / git 竞态 / 提交冲突 / 别人的改动 | 共享工作区 + 有别的写者的处境 → 写者盘点 + 单写者或独立环境方案 + 提交前 staged 复核 + 覆盖关系判定 | `dshome-crash-recovery` · `verify-integrity` | 1.0.1 |
| `recall-tuning` | 召回调优：先分三层定位（词面 / 阈值 vs 真命中分布 / 切块）→ 再核对排序量纲（任一维的"一档"不得 ≥ 另一维整条量程）；验召回必须复刻真实上工路径（cwd/project，否则假阴性）；验收读"位次"不读"命中没有" | 召不回 / 召回不准 / 写进去却召不回 / 检索不到 / 搜不到 / 命中率 / 排序不对 / top1 / 调参 / 阈值 / minScore / 切块 / 查重 / 回归基线 / 权重 / 蒸馏验收 | 一条该召回却召不回的记忆 + 真实查询 → 三层定位结论 + 量纲核对 + 真路径位次读数 + 回归集复测 | `boot-recall` · `verify-integrity` | 1.1.1 |
| `factory-hygiene` | 出厂卫生自检（双区边界）：推之前先定位**推送面**（`git ls-files --cached --others --exclude-standard`，以 git 为准）→ 门禁两条（⑨ 人维护禁词表 + ⑩ 不依赖词表的凭据/PII 形态）→ 门禁管不了的**历史维**（`git grep HEAD`）；唯一划区判据="这段能贴在公开 GitHub 上吗"；红线定义指 `AGENTS §五`/`Invariants #13` | 出厂卫生 / 双区卫生 / 出厂区 / 私有区泄漏 / 私密泄漏 / 禁词 / 禁词表 / 凭据泄漏 / 私有名 / 盘符 / 脱敏 / 去私 / 隐私红线 / 公开仓库 / 推之前 / 能公开吗 | 一批准备进出厂区的内容 → 推送面清单 + 两门禁读数（扫了几个文件/词、命中几处）+ 历史扫结论 + 改内容清单 | `import-artifact` · `verify-integrity` · `landing-audit` | 1.0.0 |
