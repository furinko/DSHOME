...
| `concurrent-writers` | 多写者共享工作区的纪律：先盘点写者（"干净"≠"没别人"）/ 共享配置单写者、并行走独立环境 / 台账 append-only / 拿不到闸就响亮失败 / 提交前复核 staged / 善后先判谁覆盖了谁 | 并发 / 多写者 / 同时改 / 抢写 / 写冲突 / 共享工作区 / 单写者 / 互斥 / 串行 / 排队 / git 竞态 / 提交冲突 / 别人的改动 | 共享工作区 + 有别的写者的处境 → 写者盘点 + 单写者或独立环境方案 + 提交前 staged 复核 + 覆盖关系判定 | `dshome-crash-recovery` · `verify-integrity` | 1.0.0 |
| `recall-tuning` | 召回调优：先分三层定位（词面 / 阈值 vs 真命中分布 / 切块）→ 再核对排序量纲（任一维的"一档"不得 ≥ 另一维整条量程）；验召回必须复刻真实上工路径（cwd/project，否则假阴性）；验收读"位次"不读"命中没有" | 召不回 / 召回不准 / 写进去却召不回 / 检索不到 / 搜不到 / 命中率 / 排序不对 / top1 / 调参 / 阈值 / minScore / 切块 / 查重 / 回归基线 / 权重 / 蒸馏验收 | 一条该召回却召不回的记忆 + 真实查询 → 三层定位结论 + 量纲核对 + 真路径位次读数 + 回归集复测 | `boot-recall` · `verify-integrity` · `evolution-checkup` | 1.1.0 |
