...
|---|---|---|---|---|---|
| `verify-integrity` | 验证可信度四则：真加载（语言层查不出"东西不存在"）/ 反例证伪（写不出反例=没验过）/ 不污染被测对象 / 输入缺失即响亮失败 | 验证 / 自测 / 怎么确认生效 / 测试全绿 / 门禁可信 / 裁判本身 / 反例 / 评分 / 打分 / 对比评分 / 判据过时 / 凭据泄漏 | 待验证机制+已有手段 → 四则判据 + 反例设计 + 前置检查 + 响亮失败改造（+5 配方：判别"线上跑的是不是新代码"、门禁探针零风险设计、调参改动先离线扫参数矩阵、评分/自评三件套、判据也是消费者＝同批改判据 + 复刻用户路径 + 诊断打码） | landing-audit · verify-host-plugins · verify-guard-decisions | 1.6.0 |
...
| `concurrent-writers` | 多写者共享工作区的纪律：先盘点写者（"干净"≠"没别人"）/ 共享配置单写者、并行走独立环境 / 台账 append-only / 拿不到闸就响亮失败 / 提交前复核 staged / 善后先判谁覆盖了谁 | 并发 / 多写者 / 同时改 / 抢写 / 写冲突 / 共享工作区 / 单写者 / 互斥 / 串行 / 排队 / git 竞态 / 提交冲突 / 别人的改动 | 共享工作区 + 有别的写者的处境 → 写者盘点 + 单写者或独立环境方案 + 提交前 staged 复核 + 覆盖关系判定 | `dshome-crash-recovery` · `verify-integrity` | 1.0.2 |
