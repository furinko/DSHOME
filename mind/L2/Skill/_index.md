...
| `concurrent-writers` | 多写者共享工作区的纪律：先盘点写者（"干净"≠"没别人"）/ 共享配置单写者、并行走独立环境 / 台账 append-only / 拿不到闸就响亮失败 / 提交前复核 staged / 善后先判谁覆盖了谁 | 并发 / 多写者 / 同时改 / 抢写 / 写冲突 / 共享工作区 / 单写者 / 互斥 / 串行 / 排队 / git 竞态 / 提交冲突 / 别人的改动 | 共享工作区 + 有别的写者的处境 → 写者盘点 + 单写者或独立环境方案 + 提交前 staged 复核 + 覆盖关系判定 | `dshome-crash-recovery` · `verify-integrity` | 1.0.3 |
