...
2. **自愈**（由易到难）：
   ...
   - **逃生**：宿主还起不来时用安全模式（`scripts/safe.mjs` 只挂白名单，id 从 cordis.patch.yml 动态解析）先起来，再修坏插件。
...
| `scripts/safe.mjs` | 崩溃逃生 | 安全模式：白名单插件 id 从 cordis.patch.yml 动态解析（加了插件无需手动同步清单；见两处硬编码数组=解析失败需排查） |
