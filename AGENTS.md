...
> 🔴 **权威版声明**：本文件（根 `E:\DSHOME\AGENTS.md`）为唯一权威版；`build-stage\payload\AGENTS.md` 为构建期打包快照，**不单独维护、禁止手改**——以根版为准。
...
- 🟡 **上工自动召回** — 每次会话开始**先召回再动手**：直接跑 `node scripts/mind-prime.mjs "<当前项目/任务>"`（机器装配 project.md 进度+下一步 + 相关 L3 + 最近教训 + user-rules 注入上下文）；改自我类文件（AGENTS/mind 规则/技能/自身记忆）前跑 `node scripts/mind-validate.mjs` 验证通过才提交。**别等用户写报告来补记忆。**
- 🔴 **指令原子性** — 用户每条指令是原子的，完成当前步骤后停下汇报，等待下一条。
- 🔴 **方案确定 ≠ 获准实施** — 改文件/构建/提交一律等待用户明确放行（"动手/开工/同意"）；方案评估、决策记录可先行。
- 🔴 **自我修改硬流程** — 改 AGENTS.md / mind 规则 / 技能 / 自身记忆等"自我类文件"，三条必须同时满足才落盘：① 已有用户**明确放行**；② 改前**快照**原文件（`E:\DSHOME\.agent-snapshot\`）；③ 改后跑 `node scripts/mind-validate.mjs` **通过**（含 Tree↔_index↔实际 同步 / 权威源单一 / AGENTS 双版本 三类结构性校验）。任一不满足 → **不落盘**；校验未过 → **回滚快照**。本方案落地的每一步也走此流程。
...
_版本：2.1 | 2026-09-06 | 语义层落地：Concepts 契约 + 自我修改硬流程 + 根版权威声明；纪律收敛：细则下沉 Power §九_
