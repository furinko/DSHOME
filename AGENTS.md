...
- 🟡 **自主洗澡+巡视** — 自己定期做（cron `self-clean` 每天零点拉起，**一次会话边洗边巡逻**）：洗澡=清中间物/无效进化回滚/轮级缓冲/pending 检查；**巡视=主动找隐患**（记忆冗余死链/规则冲突/行为信号异常/系统积压/能力缺口）→ 低危自主修+报告，高危/用户内容只列给用户定。🔴 不碰用户的东西（见 Power.md §八）。
