#!/usr/bin/env node
// scripts/verify-web-patches.mjs — 门禁：Web 产物补丁**必须还在**（2026-09-18 加）
//
// 为什么需要它（一条真实缺口）：`scripts/patch-web-assets.mjs` 的两块补丁只改 **node_modules /
// payload 产物**（不在 git 里）——① `vendor-*.js` 的 `singleTilde→false`（单个 `~` 不再变删除线）
// ② 权限档位图标表补 `mind-guard` 项（盾牌+锁）。`pnpm install` / 升级上游会把它们**抹掉**，
...
if (r.status !== 0) {
  console.error('[verify-web-patches] ❌ Web 产物补丁缺失或版本落后（单个 `~` 会被渲染成删除线 / 权限图标缺失）');
  console.error('[verify-web-patches]    修复：node scripts/patch-web-assets.mjs   （`stage-payload` 也会自动打）');
  process.exit(1);
}
console.log('[verify-web-patches] ✅ Web 产物补丁在位（singleTilde + 权限图标 mind-guard）');
