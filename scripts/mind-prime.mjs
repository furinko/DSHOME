...
// ── Learn + user-rules ────────────────────────────────────────────────────
// 2026-09-11 改造：`只取末尾 4 条` → **末尾若干条（时效）+ 按当前任务检索 top N（相关）+ 限长**。
...
// 限长：条目长短不齐，限长后总体积与旧"末尾 4 条"基本持平（实测 R1 总量 +1% 以内）。
// 2026-09-23 注入瘦身（R1 预算）又收一档 ⇒ **当前取值一律以代码为准**：末尾条数＝Learn 段的 `slice(-3)`、
//   相关条数 `LEARN_RETRIEVED`（=2）、限长 `LEARN_CLIP`（=160）；详情按需 read Learn.md。
// ⚠️ 2026-09-25 订正：本注释原写死「末尾 4 条 / top3 / 300 字」，瘦身后未跟 ⇒ 与实现、与规则侧
//   （`HUB §三` / `Ritual §一 step9` 均写"末尾 3 条 + 检索 2 条"）漂移；**写死数字的注释必然会漂**，故不再复述数字。
