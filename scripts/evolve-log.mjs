...
if (cmd === 'snapshot') {
  ...
  // 2026-09-24 降为信息行（判据①"报警必须能熄灭"，恒亮灯第四处）：`repeat-mistakes` 属**人工记账、
  //   只增不减** ⇒ 拿累计值当阈值**必然恒亮**，且与上面 :777 自己印的「人工记账**不参与报警**」口径
  //   自相矛盾；更糟的是它顺带把本命令**永久置 exit 1**（调用方再也分不清"真报警"与"旧账恒亮"）。
  //   改为 ℹ️：**审视进化仍要做**，但由**可判事件**触发（同主题复发 / 判断失误当场），不由累计阈值触发。
  if (logRows >= 8 && (m['repeat-mistakes'] || 0) >= 2) {
    console.log(`  ℹ️ 重复踩坑累计 ${m['repeat-mistakes']}（人工记账·只作趋势，**不报警**）——审视进化是否有用仍值得做，但请由可判事件触发（同主题复发 / 判断失误当场），别用累计阈值`);
  }
