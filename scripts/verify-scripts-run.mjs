...
const SAFE = new Set([
  ...
  // 2026-09-26 加：角色卡池体检器（**只读报告**：id 不可读 / 疑似重影 / 疑似一次性已交付 /
  // 正文幽灵引用 / 同 id 影子卡）——它是 `Ritual §四/§二`「角色卡池体检」那条规则的**机器件**。
  // 无参可跑（**只读、不改任何文件**）；信号数不是失败判据 ⇒ 它**永远 exit 0**（除非两个卡目录都不存在
  // ⇒ 响亮 exit 1）。⚠️ 读数随 cwd 变（判据 [4] 的解析根含 cwd）⇒ 冒烟从仓库根跑＝12 条信号属正常。
  // 它自己的 44 条断言在 `--selftest` 分支，**不在此处自动跑**（需人工/专门步骤）。
  'scripts/role-card-audit.mjs',
  ...
  // 2026-09-26 加：心智面板「卡名可区分性」——卡名取自正文标题/主题目录名 ⇒ 同主题记忆与同文档快照
  // 全部同名（主人反馈"辨识度太低"；活数据实测 292 节点里 237 个落在重名组）。无参＝只读自检
  ...
  'scripts/verify-trash-sweep.mjs',
  // 2026-09-26 加：快照「时间窗裁剪」（`evolve-log.mjs snap-prune`）的行为核验——`Memory §十一` 那条
  // 「保留最近 2 个自然日」口径此前**全仓无实现**。14 条含反例（dry-run 不许动 / 非法参数响亮失败 /
  // 不合规文件名不误伤）。临时 DSH_HOME，真仓库零触碰 · PASS 0 / FAIL 1。
  'scripts/verify-snap-prune.mjs',
  // 2026-09-26 加：记忆「压缩阶梯第 1 档·指针化」（`memory-archive.mjs`，把超配额的长 `##` 小节搬到
  // `L3\history\` + 原处留指针）的行为核验——它会**改写主人记忆正文**，没门禁＝拿真记忆当测试场。
  // 临时 DSH_HOME 的 L3 副本，真仓库零触碰 · 17 条（含"搬走的字节逐字节一致""同名不覆盖""搬不动不留垃圾"）。
  'scripts/verify-memory-archive.mjs',
